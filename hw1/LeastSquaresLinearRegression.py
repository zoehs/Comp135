    ...
    * self.b : float
    ...
    def fit(self, x_NF, y_N):
        ...
        * self.b (scalar real bias, if desired)
        ...
        temp = np.hstack((x_NF, one_col))
        print(temp.shape)
        
        x_til = np.dot(np.linalg.inv(np.dot(temp.T, temp)), np.dot(temp.T, y_N))
        
        self.w_F = x_til[:-1]
        
        self.b = x_til[-1]
        
        #self.w_F = np.delete(self.w_F, -1, axis = 1)
    ...
    def predict(self, x_MF):
        ...
        yhat_N = np.dot(x_MF, self.w_F.T) + self.b
